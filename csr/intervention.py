...
from .markov_chain import generate_recurrent_matrix

def generate_intervention_sample(start: int, p_init: float, mat: np.ndarray, seq_len: int) -> tuple:
    """
...
def generate_normal_sample(start: int, p_init: float, mat: np.ndarray, seq_len: int) -> tuple:
    """
...
def build_intervention_big_dataset(rvs: int, tr_size: int, seq_len: int, intervention_p: float = 0.8, num_processes=8) -> tuple:
    """
