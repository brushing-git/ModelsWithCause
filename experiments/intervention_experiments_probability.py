...
def build_experiment_data(dataset: np.ndarray, 
    ...
    experiment_data = np.zeros((n_samples, length))
    experiment_ps = np.zeros(n_samples)
    for i, idx in enumerate(samples):
        # Save the sample
        experiment_data[i,:] = dataset[idx]
        # Save the probabilities
        experiment_ps[i] = ps[idx]
        #experiment_ps[i] = ps[idx] + np.log(0.5) # We add log 0.5 to correct the probability
        # Check whether the end of the sequence has an intervention or not and then correct the log probabilities
        #if experiment_data[i,-1] < 6:
        #    experiment_ps[i] += np.log(1/6) + np.log(1/97)
...
def test_model(model, 
               ...
               rng_samples: int = 30) -> tuple:
    """
    Loops through an experimental dataset and computes the difference between the log probabilities of the sequence 
    and the original log probabilities for the model compared against the differences taken from random samples in the 
    experimental data.

    We use rng_samples to compute null results by randomly sampling other sequences and then looking at the difference 
    in log probabilities between the interventional distribution and the one predicted for those random sequences and 
    then take the average.

    Also returns a 3x3 wasserstein distance array.
    """
    ...
    for i in tqdm(range(samples)):
        ...
        probs_null[i] = probs_null[i] / rng_samples
    
    # Calculate the wasserstein
    wasserstein_results = calculate_wasserstein(ps=ps_dataset, probs_model=probs_model, probs_null=probs_null)

    return results_model, results_null, wasserstein_results, probs_model, probs_null
...
def build_model(model_name: str, file_path: str, test_model=False):
    ...
    if model_name == 'NADE':
        ...
        if test_model:
            print('Loading the model dictionary.')
            model.load_state_dict(torch.load(file_path, map_location=model.device))
        return model
    elif model_name == 'MOE':
...
def main():
    ...
    new_dir = "ExperimentIntervention" + MODEL + DATA_NAME + str(TEXTLENGTH)
    path = os.getcwd()
    ...
    exp_data, ps_data = build_experiment_data(dataset=dataset,
                                              length=TEXTLENGTH,
                                              ps=ps,
                                              n_samples=N_SAMPLES)
    ...
    results_model, results_null, results_wasserstein, model_probs, null_probs = test_model(model=model, exp_dataset=exp_data, ps_dataset=ps_data)
    ...
    fn = MODEL + '-' + DATA_NAME + str(TEXTLENGTH) + '-intervention_wasserstein_results'
    save_path = os.path.join(new_path, fn + '.csv')
    np.savetxt(save_path, results_wasserstein, delimiter=',')

    # Save the model probabilities and null probs
    fn = MODEL + '-' + DATA_NAME + str(TEXTLENGTH) + '-intervention_model_probs'
    save_path = os.path.join(new_path, fn + '.csv')
    np.savetxt(save_path, model_probs, delimiter=',')
    fn = MODEL + '-' + DATA_NAME + str(TEXTLENGTH) + '-intervention_null_probs'
    save_path = os.path.join(new_path, fn + '.csv')
    ...
    # Save the wasserstein
    fn = MODEL + '-' + DATA_NAME + str(TEXTLENGTH) + '-no_train_intervention_wasserstein_results'
    save_path = os.path.join(new_path, fn + '.csv')
    np.savetxt(save_path, results_wasserstein, delimiter=',')

    # Save the model probabilities and null probs
    fn = MODEL + '-' + DATA_NAME + str(TEXTLENGTH) + '-no_train_intervention_model_probs'
    save_path = os.path.join(new_path, fn + '.csv')
    np.savetxt(save_path, model_probs, delimiter=',')
    fn = MODEL + '-' + DATA_NAME + str(TEXTLENGTH) + '-no_train_intervention_null_probs'
    save_path = os.path.join(new_path, fn + '.csv')
    np.savetxt(save_path, null_probs, delimiter=',')
